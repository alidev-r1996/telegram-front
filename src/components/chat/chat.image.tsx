    ...
    <div className={`flex items-start gap-4 ${!isYou && "ml-auto"}`}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <img onClick={()=>setModal(true)} src={`http://localhost:3000/${media}`} alt="" className="rounded-lg aspect-[9/5] w-full cursor-zoom-in" />
        </div>
      ...
      {modal && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 md:w-[30%] w-[60%] aspect-[6/7] bg-slate-600 p-4 rounded-lg z-50">
          <img onClick={()=>setModal(false)} src={`http://localhost:3000/${media}`} alt="" className="rounded-lg size-full cursor-zoom-out" />
      </div>}
    </div>
  );
};
